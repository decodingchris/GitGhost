...
def test_init_creates_include_and_repo(tmp_path, runner):
    ...
    assert ".gitvaultinclude" in os.listdir()
    assert ".gitvault_private" in os.listdir()
...
def test_init_when_include_exists(tmp_path, runner):
    os.chdir(tmp_path)
    (tmp_path / ".gitvaultinclude").write_text("# test\n")
    result = runner.invoke(cli, ["init"])
    assert result.exit_code == 0
    assert ".gitvaultinclude" in os.listdir()
...
def test_save_with_message(tmp_path, runner):
    os.chdir(tmp_path)
    (tmp_path / ".gitvaultinclude").write_text("secret.txt\n")
...
def test_status_empty_include(tmp_path, runner):
    os.chdir(tmp_path)
    (tmp_path / ".gitvaultinclude").write_text("")
    runner.invoke(cli, ["init"])
...
def test_status_empty_private_repo(monkeypatch):
    ...
    with mock.patch("gitvault.cli.ensure_private_repo", return_value=repo_mock), \
         mock.patch("gitvault.cli.parse_gitvaultinclude", return_value=["file1.txt"]), \
...
def test_status_directory_recursion(monkeypatch):
    ...
    repo_mock.untracked_files = []
    with mock.patch("gitvault.cli.ensure_private_repo", return_value=repo_mock), \
         mock.patch("gitvault.cli.parse_gitvaultinclude", return_value=["dir1"]), \
         mock.patch("gitvault.cli.copy_private_files"), \
...
def test_init_gitignore_exists_without_block(monkeypatch, tmp_path):
    ...
    with mock.patch("gitvault.cli.Repo.init"), \
        ...
        assert ".gitvault_private" in content

def test_init_gitignore_exists_with_block(monkeypatch, tmp_path):
    os.chdir(tmp_path)
    (tmp_path / ".gitvaultinclude").write_text("secret.txt\n")
    (tmp_path / "secret.txt").write_text("top secret")
    (tmp_path / ".gitignore").write_text("# GitVault\n.gitvault_private/\n.gitvaultinclude\n")
    with mock.patch("gitvault.cli.Repo.init"), \
         mock.patch("subprocess.run", side_effect=FileNotFoundError):
        runner = CliRunner()
        result = runner.invoke(cli, ["init"])
        content = (tmp_path / ".gitignore").read_text()
        assert content.count(".gitvault_private") == 1  # no duplicate block
...
def test_init_private_repo_fallback(monkeypatch):
    ...
    repo_init = mock.Mock()
    with mock.patch("gitvault.cli.Repo", repo_cls), \
         mock.patch("gitvault.cli.Repo.init", repo_init), \
         mock.patch("gitvault.cli.open", mock.mock_open(), create=True), \
         mock.patch("subprocess.run", side_effect=FileNotFoundError):
        runner = CliRunner()
        result = runner.invoke(cli, ["init"])
...
def test_init_nested_exceptions(monkeypatch):
    ...
    with mock.patch("gitvault.cli.Repo", repo_cls), \
         ...
         mock.patch("gitvault.cli.Repo.create_remote", side_effect=Exception("fail")):
        runner = CliRunner()
        result = runner.invoke(cli, ["init"])
...
def test_init_get_public_repo_name_exception(monkeypatch):
    ...
    with mock.patch("gitvault.cli.Repo.init"), \
         ...
         mock.patch("gitvault.cli.Repo") as repo_mock:
...
def test_cli_entry_point():
    # Cover line 300
    from gitvault import cli as cli_module
